...
```{r}
birthweigh_df_fit = lm(bwt ~ wtgain + fincome + gaweeks + malform + mrace, data = birthweigh_df )

## regression diagnostics and exploration of our model
## example with mrace and resid
birthweigh_df %>% 
  modelr::add_residuals(birthweigh_df_fit) %>% 
  modelr::add_predictions(birthweigh_df_fit) %>% 
